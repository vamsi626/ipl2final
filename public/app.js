...
form.addEventListener('submit', (e)=>{
  yeard1 = form.elements.year.value;
  console.log(yeard1);
  ...
  fetch("./data1.json")
  .then(r => r.json())
  .then(visualizeDataDynamic);
  e.preventDefault();
 
})
function visualizeDataDynamic(data) {
  
 
  visualizeEconomyBowlersDynamic(data.expressEco);
  
  return;
}

function visualizeEconomyBowlersDynamic(e)
{
  Highcharts.chart("dynamic-economy-bowlers", {
    ...
    title: {
      text: ` Top 10 economical bowlers of ${yeard1}`
    },
    xAxis: {
      type: "category"
    },
